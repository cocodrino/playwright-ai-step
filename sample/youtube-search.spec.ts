/**
 * YouTube Search + Extract — sample test suite for playwright-ai-step
 *
 * Demonstrates the full chain: search → extract video list →
 * iterate and extract comments per video.
 *
...
// ─── YouTube: search + extract videos ───────────────────────────────────

test('search YouTube and extract structured video list', async ({ page }) => {
    ...
    'extract every video result on this page',
    {
      ...
      schema: {
        videos: [{
          ...
          views: 'string — view count in human format (e.g. "1.2M views", "340K views")',
...
// ─── YouTube: extract videos then comments from each ─────────────────────

test('extract video list and comments from each video', async ({ page }) => {
  ...
  // Step 2: iterate over each video and extract comments
  // (limit to first 2 to keep the test fast)
  for (const video of searchResults.videos.slice(0, 2)) {
    ...
    for (const c of comments.comments.slice(0, 3)) {
      console.log(`      @${c.username}: ${c.text.slice(0, 80)}${c.text.length > 80 ? '…' : ''}`)
      console.log(`      ❤️  ${c.likes} likes`)
    }
  }
})

// ─── YouTube: basic navigation + assertions ─────────────────────────────

test('search, filter, and validate results appear', async ({ page }) => {
  await page.goto('https://www.youtube.com')

  await ai('click the search box', { page })
  await ai('type "React crash course" in the search box', { page })
...
test('navigate to trending and validate content', async ({ page }) => {
  await page.goto('https://www.youtube.com')
